...
async fn download_file_to_path(
    ...
    fs::create_dir_all(PathBuf::from(path.clone()).parent().unwrap())
        .await
        .expect("Failure creating download folder");
    ...
    while let Some(item) = stream.next().await {
        ...
        app.emit("file_download_progress", progress).unwrap();
    }
    ...
    set_permissions(&path, file_permissions)
        .await
    ...
    let actual_hash = compute_sha256(&path)
        .await
        .map_err(|_| format!("Failed to compute SHA-256 hash of `{}` file", &path))?;
    if actual_hash != hash {
        println!(
            "Error: SHA-256 hash of `{}` file does not match expected hash",
            &path
        );
